...
class SVAMediaSkill(SimpleVoiceAssistant):
    ...
    def __init__(self, bus=None, timeout=5):
        ...
        self.log.debug(
            f"SVAMediaSkill.__init__() skill_id: {self.skill_id} skill_base_dir: {self.skill_base_dir}"
        )
    ...
    def handle_oob_detected(self, msg):
        ...
        if self.active_media_skill == None:  # nothing is playing
            ...
            self.log.debug(
                f"SVAMediaSkill.handle_oob_detected(): oob_type = {oob_type}"
            )
            match oob_type:
                case "previous":
                ...
                case _:
                    self.log.error(
                        f"SVAMediaSkill.handle_oob_detected() unexpected OOB: {oob_type}"
                    )
    ...
    def handle_register_media(self, msg):
        ...
        if skill_id not in self.media_skills:
            ...
            self.log.debug(
                f"SVAMediaSkill.handle_register_media() registered media skill: {skill_id}"
            )
        else:
            self.log.warning(
                f"SVAMediaSkill.handle_register_media() {skill_id} already registered"
            )
    ...
    def handle_media_response(self, msg):
        ...
        self.log.info(
            f"SVAMediaSkill.handle_media_response() media skill {skill_id} going active"
        )
    ...
    def handle_query(self, msg):
        ...
        for skill_id in self.media_skills:
            self.log.debug(
                f"SVAMediaSkill.handle_query(): sent media_get_confidence to {skill_id}"
            )
            info = {
    ...
    def handle_command(self, msg):
        ...
        self.log.debug(
            f"SVAMediaSkill.handle_command(): active media is {self.active_media_skill}"
        )
    ...
    def handle_message(self, msg):
        ...
        self.log.debug(
            f"SVAMediaSkill.handle_message(): active media: {self.active_media_skill} msg: {msg}"
        )
        ...
        if subtype == "media_register_request":
            ...
            if (
                ...
                and subtype == "media_player_command_response"
                and response == "session_ended"
            ):
                self.log.debug(
                    f"SVAMediaSkill.handle_message() media session ended for {self.active_media_skill}"
                )
                self.active_media_skill = None
            else:
                self.log.warning(
                    f"SVAMediaSkill.handle_message() unrecognized subtype: {subtype}"
                )
