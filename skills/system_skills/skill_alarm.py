...
class AlarmSkill(SimpleVoiceAssistant):
  def __init__(self, bus=None, timeout=5):
    ...
    self.alarms = []                       # reset alarm database
    ...
    self.log.info(f"AlarmSkill.__init__() Initialized: {self.alarms}")

  def handle_message(self, msg):
    if msg["payload"]["subtype"] == "oob_detect" and msg["payload"]["verb"] == "snooze":
      # snooze means we create a new alarm for 5 minutes from now and then we go dead
  ...
  def handle_create(self, msg):
    self.log.debug("AlarmSkill.handle_create: {msg}")
    ...
    if new_alarm_time == "":
      ...
      if new_alarm_time is not None:
        ...
        if response:
          ...
          self.speak(resp) 
          return True
        else:                              # error probably duplicate so inform the user
