...
class ConnectivitySkill(SimpleVoiceAssistant):
  ...
  def run(self):
    ...
    while self.is_running:
      # this is a long running skill. it needs to maintain state because you do not want to keep bothering the user every 
      # 90 seconds because you are still down, but you also want to periodically retry to establish connectivity. For now
      # audibly notify the user when you change state in either direction.
      MAX_FAILS_ALLOWED = 3
