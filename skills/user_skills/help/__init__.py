...
class HelpSkill(SimpleVoiceAssistant):
    ...
    def handle_message(self, msg):
        if msg["payload"]["subtype"] == "oob_detect":
            ...
            self.speak(
                "You can say things like general help, or help with alarms or ask me for a list of topics."
            )
            self.get_user_input(self.handle_help_topic)

    def handle_help_topic(self, user_response):
        # This callback is triggered when the user replies
        self.speak(f"Playing help for {user_response}")
