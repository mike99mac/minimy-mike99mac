...
class WeatherSkill(SimpleVoiceAssistant):
  def __init__(self, bus=None, timeout=5):
    ...
    response = execute_command("curl ifconfig.me") # get public IP address
    ra = response.split("\n")
    start_indx = ra[0].find(":") + 1
    self.my_public_ip = ra[0][start_indx:]
    response = execute_command("curl https://ipinfo.io/" + self.my_public_ip) # get geo data associated with public ip
  ...
  def handle_msg(self, msg):
    if self.busy:
      ...
      return
    loc_city = ""                          # if no location in the request, use default location
    loc_state = ""
    ...
    si = page.find(tag, start_indx)        # debug
    #location_name = self.get_tag_data(page, tag, "</h2>", start_indx)
    location_name = self.get_tag_data(page, tag, "<", start_indx+2)
    location_name = location_name.replace("\n","")
    location_name = location_name.strip()
    state_abbrev = location_name[len(location_name) -2:] # convert state abbreviation to state name
