...
class TTSEngine:
  ...
  def handle_event(self, event, msg):
    self.log.info(f"TTS Engine handle_event() state: {self.state} event: {event} owner: {self.current_session.owner} tts_sid: {self.current_session.tts_sid} msid: {self.current_session.msid}")
  ...
  def _active_speak(self, msg):
    if msg['skill_id'] != self.current_session.owner:
      ...
      return False
    # otherwise, add the text to the active speak queue
