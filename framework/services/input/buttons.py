...
"""
 ...
 GPIO pin layout - odd pins are toward the inside of the Pi:
  ...
  How to get stdin to buttons process while in the background:
...
class Buttons(SimpleVoiceAssistant):  
  ...
  def monitor_buttons(self):
    ...
    GPIO.add_event_detect(self.prev_button, GPIO.FALLING, callback=self.button_released, bouncetime=300)
    GPIO.add_event_detect(self.stop_button, GPIO.RISING, callback=self.stop_button_pressed, bouncetime=300)
    GPIO.add_event_detect(self.next_button, GPIO.FALLING, callback=self.button_released, bouncetime=300)
  ...
  def stop_button_pressed(self, channel):
    """
    Save the time when the stop button was pressed, then reset the event detect to trap when the button is released
    """
    self.button_start_time = time.time()
    self.log.debug(f"Buttons.stop_button_pressed() channel: {channel} button_start_time: {self.button_start_time}") 
    GPIO.remove_event_detect(self.stop_button)
    GPIO.add_event_detect(self.stop_button, GPIO.FALLING, callback=self.stop_button_released, bouncetime=300)  

  def stop_button_released(self, channel):
    """
    If stop button was held for more than 2 seconds, perform a "stop", otherwise "pause/resume"
    """
    self.press_duration = time.time() - self.button_start_time
    self.log.debug(f"Buttons.stop_button_released() channel: {channel} press_duration: {self.press_duration}") 
    if self.press_duration > 20:            # button was held for more than 2 seconds
  ...
  def button_released(self, channel):
    ...
    """
    self.log.debug(f"Buttons.button_released() channel: {channel}") 
    match channel:
      case self.prev_button:
        self.log.debug(f"Buttons.button_released() previous")
        self.send_message("previous")
      case self.next_button:
        self.log.debug(f"Buttons.button_released() next")
        self.send_message("next")
      case _:                              # not expected
        self.log.error(f"Buttons.button_released() UNEXPECTED channel: {channel}")
